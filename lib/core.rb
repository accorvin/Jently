module Core
  def Core.test_pull_request(pull_request_object)
    begin
      ...
      pull_request = PullRequestsData.read[repo_name][pull_request_id]
...
#        Git.setup_testing_branch(pull_request)
        ...
        Github.set_pull_request_status(repo_name, pull_request_id, {:status => 'pending', :description => 'Jenkins build started.'})
        job_id = Jenkins.start_job(jenkins_job_name, pull_request_id, pull_request_data[:head_branch])
        state = Jenkins.wait_on_job(jenkins_job_name, job_id)

        Github.set_pull_request_status(repo_name, pull_request_id, thr.value)
#        if timeout
#          Github.set_pull_request_status(pull_request_id, {:status => 'error', :description => 'Jenkins build timed out.'})
#        else
#          Github.set_pull_request_status(pull_request_id, thr.value)
#        end
      end
    rescue => e
...
#      Github.set_pull_request_status(pull_request_id, {:status => 'error', :description => 'A Jenkins build error has occurred. This pull request will be automatically rescheduled for testing.'})
    ...
    if !open_pull_requests.empty?
      pull_request_to_test = PullRequestsData.get_pull_request_id_to_test(open_pull_requests)
    end
